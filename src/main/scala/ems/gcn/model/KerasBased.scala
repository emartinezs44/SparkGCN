...
import com.intel.analytics.bigdl.dllib.keras.layers.{
  Dense,
  Dropout,
  GraphConvolutionK
}
...
object KerasBased {
  def getModel(
    ...
    model.add(Dropout[Float](dropout, inputShape = Shape(inputSize)))
    model.add(GraphConvolutionK[Float](matrix, batchSize, inputSize))
    model.add(
      Dense[Float](
        intermediateSize,
        activation = "relu",
        wRegularizer = L2Regularizer(5e-4)
      ).setName("layer-1")
    )
    model.add(GraphConvolutionK[Float](matrix, batchSize, intermediateSize))
    model.add(
      Dense[Float](labelsNumber, activation = "log_softmax").setName("layer-2")
    )
